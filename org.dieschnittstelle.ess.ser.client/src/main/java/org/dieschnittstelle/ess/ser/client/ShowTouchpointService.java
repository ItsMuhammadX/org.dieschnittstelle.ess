...
public class ShowTouchpointService {
	...
	public void deleteTouchpoint(AbstractTouchpoint tp) {
		...
		try {
			...
			if(response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				show("touchpoint deleted successfully");
			}
			else if(response.getStatusLine().getStatusCode() == HttpStatus.SC_NOT_FOUND) {
				show("touchpoint not found");
			} else{
				show("touchpoint not deleted");
			}
	...
	public AbstractTouchpoint createNewTouchpoint(AbstractTouchpoint tp) {
		...
		try {
			...
			ObjectOutputStream oos = new ObjectOutputStream(baos); // wandelt es in einem Binärformat um
			...
			if(response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				...
				AbstractTouchpoint reveivedTp = (AbstractTouchpoint) ois.readObject();
				show("receivedTp: %s",  reveivedTp);

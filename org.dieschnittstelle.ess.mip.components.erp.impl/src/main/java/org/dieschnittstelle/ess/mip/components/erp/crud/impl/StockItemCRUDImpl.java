...
public class StockItemCRUDImpl implements StockItemCRUD{
    ...
    public List<StockItem> readStockItemsForProduct(IndividualisedProductItem prod) {
        Query query = em.createQuery("SELECT DISTINCT si FROM StockItem si WHERE si.product.id = :prodId", StockItem.class);
        query.setParameter("prodId", prod.getId());
    ...
    public List<StockItem> readStockItemsForPointOfSale(PointOfSale pos) {
        Query query = em.createQuery("SELECT DISTINCT si FROM StockItem si WHERE si.pos.id = :posId", StockItem.class);
        query.setParameter("posId", pos.getId());
        return query.getResultList();
